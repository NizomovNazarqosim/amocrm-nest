...
export class AmoCRMApiClient {
  ...
  CREATE_CONTACT = async (contactData: CreateUserDto) => {
    try {
    ...
    try {
      ...
      const request = new Request(endpoint, {
        headers: {
          ...this.headers,
          'Content-Type': 'application/json',
          Authorization: `Bearer ${process.env.AMOCRM_LOGIN}`,
        },
      });
  ...
  UPDATE_CONTACT = async (id: number, createData: CreateUserDto) => {
    try {
      ...
      const payload: any = {
        ...
        custom_fields_values: [
          {
            field_id: 756829,
            values: [{ value: createData.email }],
          },
          {
            field_id: 757025,
            values: [{ value: createData.phone }],
          },
        ],
      };
